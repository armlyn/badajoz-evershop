...
function Actions({ regions = [], selectedIds = [] }) {
  ...
  const deleteRegions = async () => {
    ...
    const promises = regions
      .filter((region) => selectedIds.includes(region.uuid))
      .map((region) => axios.delete(region.deleteApi));
  ...
  const actions = [
    {
      ...
      onAction: () => {
        openAlert({
          heading: `Delete ${selectedIds.length} regions`,
          ...
          <tr>
            <th className="align-bottom">
              <Checkbox
                onChange={(e) => {
                  if (e.target.checked) {
                    setSelectedRows(regions.map((c) => c.uuid));
                  } else {
                    setSelectedRows([]);
                  }
                }}
